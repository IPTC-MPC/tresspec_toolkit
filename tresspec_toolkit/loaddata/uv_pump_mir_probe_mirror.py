...
def uv_pump_mir_probe(wrkpath, discard_stitching_blocks=[], load_processed_data=True):

    """
    invoke uv_pump_mir_probe to load data recorded on the UV-pump-mIR-probe experiment
        ...
        for i in range(no_scans):
            ...
            for k in range(no_sb):
                if k not in bad_sb:
                    ...
                    delays_sb = data[lower_index:upper_index + 1, 0]
